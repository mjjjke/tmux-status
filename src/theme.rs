...
/// Main foreground for regular status text. dotbar `fg-current`.
pub const TEXT_FG: Color = Color::Rgb(0xc0, 0xca, 0xf5); // #C0CAF5

/// Muted foreground for secondary status text. dotbar `fg`.
pub const MUTED_FG: Color = Color::Rgb(0x56, 0x5f, 0x89); // #565F89 comment

/// Separator / divider glyph color.
pub const SEPARATOR: Color = MUTED_FG;

/// Clock time (HH:MM) foreground: bright, so it stands out.
pub const CLOCK_TIME_FG: Color = TEXT_FG;

/// Clock date (dd-Mon-yy) foreground: muted, recedes vs the time.
pub const CLOCK_DATE_FG: Color = MUTED_FG;
